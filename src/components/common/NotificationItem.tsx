...
export const NotificationItem = ({ n, onMarkRead }: { n: Notification; onMarkRead?: () => void }) => {
  ...
  return (
    <div className={cn('flex gap-3 p-4 rounded-xl border border-border/70 bg-card', !n.read && 'border-primary/30 bg-primary/5')}>
      ...
      {!n.read && onMarkRead && (
        <Button variant="ghost" size="sm" onClick={onMarkRead} className="self-start">تم القراءة</Button>
      )}
    </div>
  );
};
