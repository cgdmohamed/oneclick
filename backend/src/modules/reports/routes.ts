...
router.get('/overview', async (req, res, next) => {
  try {
    ...
    const totals = await t.db.query(`
      ...
      FROM invoices
    `);
    const lowStock = await t.db.query(`SELECT COUNT(*)::int AS n FROM products WHERE quantity <= alert_level`);
    const clients  = await t.db.query(`SELECT COUNT(*)::int AS n FROM clients`);
...
router.get('/aging', async (req, res, next) => {
  try {
    ...
    const rs = await t.db.query(`
      ...
      FROM invoices WHERE remaining > 0
    `);
