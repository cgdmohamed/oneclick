-- Migration 052: Include the company's invoice appearance settings (template,
-- accent color, terms, footer) in the public invoice payload — the public
-- shared-link invoice previously fell back to a fixed generic look, drifting
-- from what the company actually configured in Settings and from the invoice
-- view inside the app.
...
BEGIN
  SELECT i.id, i.number, i.public_id, i.issue_date, i.due_date,
         ...
         i.company_id, i.client_id, i.qr_public_visible
  INTO v_invoice
  FROM invoices i
  ...
  SELECT c.name, c.email, c.phone, c.address,
         ...
         c.invoice_template, c.invoice_accent_color,
         c.invoice_terms, c.invoice_footer
  INTO v_company
  ...
  RETURN jsonb_build_object(
    'invoice', jsonb_build_object(
      ...
      'invoice_template',      v_company.invoice_template,
      'invoice_accent_color',  v_company.invoice_accent_color,
      'invoice_terms',         v_company.invoice_terms,
      'invoice_footer',        v_company.invoice_footer
    ),
