...
import { Pencil, Plus, Briefcase, Trash2 } from 'lucide-react';
...
const empty = (): Project => ({ id: '', code: '', name: '', description: '', isActive: true, createdAt: new Date().toISOString() });

const Projects = () => {
  const { list, save, remove } = useResource<Project, ProjectRow>({
    path: '/api/projects',
    key: 'projects',
    initial: [],
    fromRow: (r) => ({ id: r.id, code: r.code ?? '', name: r.name, description: r.description ?? '', isActive: r.is_active, createdAt: r.created_at }),
    toRow: (p) => ({ code: p.code || null, name: p.name, description: p.description || null, is_active: p.isActive }),
  });
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Project>(empty());

  const submit = async () => {
    if (!editing.name.trim()) return toast.error('اسم المشروع مطلوب');
    await save(editing);
    setOpen(false);
  };
  ...
  const columns: Column<Project>[] = [
    ...
    { key: 'name', header: 'المشروع', cell: (r) => <span className="font-medium">{r.name}</span> },
  ...
  return (
    <div>
      <PageHeader title="المشاريع" description="إدارة المشاريع لربطها بالفواتير والقيود وتقارير ربحية المشاريع"
        actions={<Button onClick={() => { setEditing(empty()); setOpen(true); }}><Plus className="h-4 w-4 ml-1" /> مشروع جديد</Button>} />
      <DataTable data={list} columns={columns} searchKeys={['code', 'name']} searchPlaceholder="ابحث بالكود أو اسم المشروع..." />

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Briefcase className="h-5 w-5" /> {editing.id ? 'تعديل مشروع' : 'مشروع جديد'}</DialogTitle></DialogHeader>
          <div className="grid sm:grid-cols-2 gap-4">
            <div><Label>الكود</Label><Input className="mt-1.5" value={editing.code} onChange={(e) => setEditing(v => ({ ...v, code: e.target.value }))} /></div>
            <div><Label>اسم المشروع *</Label><Input className="mt-1.5" value={editing.name} onChange={(e) => setEditing(v => ({ ...v, name: e.target.value }))} /></div>
